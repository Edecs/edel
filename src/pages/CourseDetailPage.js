  ...
  const fetchCourseDetails = useCallback(async () => {
    try {
      ...
      if (!courseSnapshot.exists()) {
        setError(t("courseDetail.courseNotFound"));
        setCourse(null);
        return;
      }
      ...
      setError(t("courseDetail.fetchError"));
      setCourse(null);
    } finally {
      setLoading(false);
    }
    ...
    <div className="course-detail">
      <header>
        <h1 className="header-h1">
          {course ? course.name : t("courseDetail.loading")}
        </h1>
      </header>
          ...
          <div className="course-detail-content">
            {Object.values(course.subCourses || {}).filter(
              ...
              <p className="no-sub-courses">
                {t("courseDetail.noSubCourses")}
              </p>
            )}
          </div>
        ) : (
          <p>{t("courseDetail.noCourseDetails")}</p>
        )}
      </div>
    </div>
  );
